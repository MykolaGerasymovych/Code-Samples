...
class Sprite:
    ...
    def collide(self, other_object):
        distance = [self.pos[0] - other_object.get_position()[0], self.pos[1] - other_object.get_position()[1]]
        if math.hypot(distance[0], distance[1]) < self.radius + other_object.get_radius():
            return True
        else:
            return False
...
def draw(canvas):
    ...
    if group_group_collision(rock_group, missile_group) == True:
        group_group_collision(rock_group, missile_group)        
        score += 10
...
# Handler that stops and rewinds the soundtrack when the frame is closed 
def unload():
    print "Oh my, you closed the frame.\nHave you got bored so quickly?\nAllright, have a nice day!"
    soundtrack.pause()
    soundtrack.rewind()
        
# helper function to draw the rocks from the set
...
# helper function to remove collided objects     
def group_collide(group, other_object):
    ...
    for object in set(group):
        if object.collide(other_object):
            removal.add(object)
            group.difference_update(removal)
            explosion = Sprite(object.get_position(), [0, 0], 
...
def group_group_collision(group1, group2):
    ...
    for object in group1:
        if group_collide(group2, object) == True:
            group_collide(group2, object)
            removal.add(object)
            group1.difference_update(removal)
            return True
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        ...
        distance = [rock_pos[0] - my_ship.get_position()[0], rock_pos[1] - my_ship.get_position()[1]]
        if math.hypot(distance[0], distance[1]) > (asteroid_info.get_radius() + my_ship.get_radius()) * 1.5:
